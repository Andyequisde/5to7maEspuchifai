...
DELIMITER $$
CREATE PROCEDURE altaAlbun(unIdAlbun SMALLINT,
				unIdBanda SMALLINT,
                           		    	unNombre VARCHAR(45),
                          			unLanzamiento DATE,
				unCantRepro INT)
...
DELIMITER $$
CREATE PROCEDURE altaCancion(unIdCancion SMALLINT,
				     unIdAlbun SMALLINT,
                           		      	     unNombre VARCHAR(45),
                            		      	     unNro_Orden TINYINT UNSIGNED,
				    unCantRepro INT)
...
END $$
DELIMITER $$
CREATE PROCEDURE Reproducir(unIdReproduccion SMALLINT,
				       unIdCliente SMALLINT,
                            		    	       unIdCancion SMALLINT)
...
DELIMITER $$
CREATE PROCEDURE registarCliente(unIdCliente SMALLINT,
					unNombre VARCHAR(45),
                                 			unApellido VARCHAR(45),
                                 			unEmail VARCHAR(45),
                                			unaContrasena CHAR(65))
BEGIN
...
DELIMITER $$
CREATE FUNCTION CantidadReproduccionesBanda(unIdBanda SMALLINT,
						     fecha1 DATETIME,
                                             			     fecha2 DATETIME) RETURNS SMALLINT
BEGIN
